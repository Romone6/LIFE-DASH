...
const downloadPhoto = async (supabase: any, bucket: string, path: string, dest: string) => {
...
const uploadFile = async (supabase: any, bucket: string, path: string, filePath: string) => {
...
const processJob = async (supabase: any, env: ReturnType<typeof loadEnv>, job: { id: string; photo_set_id: string }) => {
  ...
  for (const photo of photos) {
    ...
    await downloadPhoto(
      supabase,
      env.TWIN_BUCKET_PHOTOS,
      photo.storage_path,
      join(workspace.imageDir, filename)
    );
  }
  ...
  if (env.WORKER_DRY_RUN) {
    ...
    await runCommandSequence(
      commands.map((command) => ({ cmd: colmapBinary, args: command.args })),
      runCommand
    );
  }
...
const runWorker = async () => {
  ...
  const poll = async () => {
    ...
    if (!next || !next.photo_set_id) return;
